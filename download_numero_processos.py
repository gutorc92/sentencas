...
class Extract_Numbers:
    ...
    def save_processos(self, id, names):
        if self.num + len(names) < 1000 and self.arquivo is not None:
        ...
        with open(os.path.join("numero_processos", arquivo), "a") as f:
            for n in names:
                f.write(n)
                f.write("\n")
        self.num += len(names)

    def get_numeros_processos(self, id, paginaIn, paginaFinal):
        session = req.Session()
        ...
        if response.status_code != req.codes.ok:
            logging.info("A url inicial nao pode se encontrada para a thread: %s", str(id))
            return 1


        self.extrai_numero_processo(id, response)
    ...
    def download_numero_processos(self):
        x = list(range(0, 11))
        pagin = [t * 10000 for t in x]
        pagout = [t - 1 for t in pagin[1:]]
        pagout.append(110000)
        for id, pagi, pago in zip(x, pagin, pagout):
            print(id, pagi, pago)
            self.get_numeros_processos(id, pagi, pago)
